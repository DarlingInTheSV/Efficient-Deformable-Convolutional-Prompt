...
class Memory(object):
    ...
    def __init__(self, size, dimension=   3 * 18):
        self.memory = {}
        self.size = size
    ...
    def push(self, keys, logits):
        for i, key in enumerate([keys]):
            ...
            self.memory.update(
                {key.reshape(self.dimension).tobytes(): logits})
    ...
    def get_neighbours(self, keys, k):
        ...
        keys = keys.reshape(-1, self.dimension)
...
class Memory2(object):
    ...
    def push(self, keys, logits): # (offset, data prompt)
        for i, key in enumerate([keys]):
            if len(self.memory.keys()) > self.size:
                self.memory.pop(list(self.memory)[0])
