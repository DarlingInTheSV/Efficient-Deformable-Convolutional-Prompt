...
class OPTIC_dataset(data.Dataset):
    ...
    def __getitem__(self, item):
        ...
        if self.img_normalize:
            img_npy, max_val, min_val = normalize_image_to_0_1(img_npy)
        label_npy = np.array(label)  # mask : 0, 128, 255.  0, 128代表两种不同的类别，我们分别置为2, 1，忽略255(白色背景),其他置为0

        mask = np.zeros_like(label_npy)
        mask[label_npy < 255] = 1
        mask[label_npy == 0] = 2
        return img_npy, mask[np.newaxis], img_file, max_val, min_val
