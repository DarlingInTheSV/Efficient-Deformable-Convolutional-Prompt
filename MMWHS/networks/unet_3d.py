...
import torch
import time
from MMWHS.networks.convert_3d_BN import AdaBN, convert_encoder_to_target, convert_decoder_to_target
...
class Conv3DBlock(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, bottleneck=False) -> None:
        ...
        # self.bn1 = nn.InstanceNorm3d(out_channels // 2)
...
class UpConv3DBlock(nn.Module):
    ...
    def __init__(self, in_channels, res_channels=0, last_layer=False, num_classes=None) -> None:
        ...
        # self.bn = nn.InstanceNorm3d(in_channels // 2)
...
class UNet3D(nn.Module):
    ...
    def __init__(self, in_channels, num_classes, level_channels=[64, 128, 256], bottleneck_channel=512, convert=False,warm_n=500) -> None:
        ...
        if convert:  # 核心科技：重写一个继承自原来BN的模块，这样原来的load_dict方法就会一直存在，这样就可以存储source训练时保留的BN信息
            convert_encoder_to_target([self.a_block1, self.a_block2, self.a_block3, self.bottleNeck], self.newBN,verbose=False,warm_n=warm_n)
            convert_decoder_to_target([self.s_block3, self.s_block2, self.s_block1], self.newBN,verbose=False,warm_n=warm_n)
            # self.up1, self.up2, self.up3, self.up4, self.bnout = convert_decoder_to_target(
            #     [self.up1, self.up2, self.up3, self.up4, self.bnout], newBN, start=0, end=5, verbose=False,
            #     warm_n=warm_n)
        #
...
if __name__ == '__main__':
    ...
    model = UNet3D(in_channels=1, num_classes=4).cuda()
    input = torch.randn((1,1,144,144,144)).cuda()
    output = model(input)
